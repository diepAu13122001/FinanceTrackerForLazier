...
// ── Inner component — chỉ render khi đã xác nhận isPlus ──────────────────────
const GoalsContent = () => {
    const navigate = useNavigate()
    const { data: goals, isLoading } = useActiveGoals()  // fetch chỉ khi Plus
            ...
            {topGoals.map(goal => {
                ...
                return (
                    <div
                        key={goal.id}
                        className={`
                            rounded-xl p-3 cursor-pointer hover:opacity-80 transition-opacity
                            ${goal.type === 'DEBT' && goal.overLimit
...
// ── Main widget — kiểm tra plan TRƯỚC, không fetch nếu Free ──────────────────
    ...
    return (
        <div className={DS.card}>
            <div className="flex items-center justify-between mb-3">
                ...
                {/* Chỉ hiện link "Xem tất cả" cho Plus user */}
                {isPlus && (
                    <button
                        onClick={() => navigate('/goals')}
